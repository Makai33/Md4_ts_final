...
export default {
  create: async function (formData: FormData) {
    return await axios.post(
      import.meta.env.VITE_APP_SERVER_HOST_API + "products",
      formData,
      {
        headers: {
  ...
  findAll: async function (maxItemPage: number, skipItem: number) {
    return await axios.get(
      ...
      }/products?maxItemPage=${maxItemPage}&skipItem=${skipItem}`
    );
  },
  search: async function (search: string) {
    return await axios.get(
      `${
        import.meta.env.VITE_APP_SERVER_HOST_API
      }/products/produceName?name=${search}`
    );
  },
  ...
  findByCategory: async function (categoryId: number) {
    return await axios.get(
      `${import.meta.env.VITE_APP_SERVER_HOST_API}/collections/${categoryId}`
    );
  },
};
